...
const Partners = ({ validatedPartnerLogos }) => {
  ...
  const partnerLogoElements = Array.isArray(validatedPartnerLogos.field_logos)
    ? validatedPartnerLogos.field_logos.map((logo) => (
        <div
          key={logo.id}
          className="p-6 flex items-center justify-center flex-grow-0 flex-shrink-0 flex-basis-1/2 max-w-1/2"
        >
          <Link href={logo.field_link.full_url} target="_blanck">
            <Image
  ...
  return (
    <div>
      <div className="flex flex-wrap justify-center">{partnerLogoElements}</div>
    </div>
  );
};
