...
export async function getValidatedPartnerLogos(context) {
  // fetching data for partner_logos content type, there should be only one content of that type.
  const logos = await drupal.getResourceCollectionFromContext<DrupalNode[]>(
    ...
    context,
    {
      params: {
        "filter[status]": 1,
        "fields[node--partners_logos]": "title,field_logos",
